...
class PFMiltonFilter : MiltonFilter() {
    ...
    if (sslSessionUser != null) {
      ...
      restAuthenticationUtils.basicAuthentication(
        ...
        if (authenticatedUser == null) {
          val msg = "Can't authenticate user '$userString' by given token. User name and/or token invalid (requestUri=${
  ...
  override fun doFilter(request: ServletRequest, response: ServletResponse, chain: FilterChain) {
    ...
    if (!DAVMethodsInterceptor.handledByMiltonFilter(request.method, request.requestURI)) {
      if (log.isDebugEnabled) {
        log.debug(
          "Request is not for us (neither CalDAV nor CardDAV-call), processing normal filter chain (requestUri=${
            request.requestURI
          })..."
        )
      }
      ...
      log.info("Request with method=${request.method} for Milton (requestUri=${request.requestURI})...")
      restAuthenticationUtils.doFilter(LegacyServletRequest(request),
        LegacyServletResponse(response),
        UserTokenType.DAV_TOKEN,
